...
if [ "$1" == "" ] || [ "$2" == "" ]; then
    echo -e "Usage:  $0 fileOfPeakfileNames.txt outfilename"
    exit 1
fi
...
do
    ...
    is_gz=`basename $pks | grep '\.bed\.gz$' | wc -l`
    is_starch=`basename $pks | grep '\.starch$' | wc -l`
    is_bed=`basename $pks | grep '\.bed$' | wc -l`
	...
	bedops -n -1 $pks $ENCODE_AWG_BLACKLIST \
	    | awk -v p=$proj '{print $1"\t"$2"\t"$3"\t"p"\t"$8}' - \
	    >> $tmp
	N=`unstarch $pks | wc -l`
    elif [ "is_bed" == "1" ]; then
