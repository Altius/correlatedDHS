      ...
      if (!vecFromVecString(p, promVec))
        {
          cerr << "Error:  On line " << linenum << ", the promoter\'s tag counts are absent, "
               << "or something else is up.  Exiting..." << endl
               << endl;
          return false;
        }
...
int main(int argc, const char* argv[])
{
  ...
  if (!ifs)
    {
      cerr << "Error:  Unable to open file \"" << argv[1] << "\" for read." << endl
           << endl;
      return -1;
    }
  ofstream ofs(argv[2]);
  if (!ofs)
    {
      cerr << "Error:  Unable to open file \"" << argv[2] << "\" for write." << endl
           << endl;
      return -1;
    }
